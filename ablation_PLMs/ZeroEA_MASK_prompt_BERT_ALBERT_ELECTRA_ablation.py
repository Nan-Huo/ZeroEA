...
if model_chose == 'ALBERT':
    PLM_model = 'albert-base-v2'
    tokenizer = AlbertTokenizer.from_pretrained(PLM_model)
    model = AlbertModel.from_pretrained(PLM_model, output_hidden_states=True)

### ELECTRA
if model_chose == 'ELECTRA':
    PLM_model = "google/electra-base-discriminator"
    tokenizer = ElectraTokenizer.from_pretrained(PLM_model)
    model = ElectraModel.from_pretrained(PLM_model, output_hidden_states=True)
