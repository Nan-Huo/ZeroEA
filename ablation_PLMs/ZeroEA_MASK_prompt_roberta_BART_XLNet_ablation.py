...
def knowledge_injection(ent_text, tokenizer, model,):
    ...
    mask_txt = mask_token + " is identical with " + ent_text +'. '
    if cut_pos < 10:
...
#             if rank_index < top_k[j] or entity_text_left[i] == entity_text_right[i]:  # We assume same entity txt to be aligned
#                 top_lr[j] += 1
                
#             ###: add failure cases
#             if (j == 0 and sim[i, rank[0]] > 0.5) or (j == 0 and rank_index >= top_k[j] and entity_text_left[i] != entity_text_right[i]):
...
#         rank_index = np.where(rank == i)[0][0]
#         for j in range(len(top_k)):
#             if rank_index < top_k[j] or entity_text_left[i] == entity_text_right[i]:
#                 top_rl[j] += 1
                
#             ### add
#             if (j == 0 and sim[rank[0], i] > 0.5) or (j == 0 and rank_index >= top_k[j] and entity_text_left[i] != entity_text_right[i]):
...
# #     with open('left-right_fail_case_15000_dbp.json', 'w') as f_lr:
# #         json.dump(lr_fail_case_dict, f_lr)
# #     with open('right-left_fail_case_15000_dbp.json', 'w') as f_rl:
# #         json.dump(rl_fail_case_dict, f_rl)
...
if model_chose == 'BART':
    ...
    cls_token = '<s>'
    sep_token = '</s>'
    tokenizer = BartTokenizer.from_pretrained(PLM_model)
    model = BartModel.from_pretrained(PLM_model, output_hidden_states=True)

## XLNet
if model_chose == 'XLNet':
    PLM_model = 'xlnet-base-cased'
    mask_token = '<mask>'
    cls_token = '<cls>'
    sep_token = '</s>'
    tokenizer = XLNetTokenizer.from_pretrained(PLM_model)
    model = XLNetModel.from_pretrained(PLM_model, output_hidden_states=True)

## xlm-RoBERTa
if model_chose == 'xlm':
    PLM_model = 'xlm-roberta-base'
    mask_token = '<mask>'
    cls_token = '<s>'
    sep_token = '</s>'
    tokenizer = XLMRobertaTokenizer.from_pretrained(PLM_model)
    model = XLMRobertaModel.from_pretrained(PLM_model, output_hidden_states=True)

### RoBERTa
if model_chose == 'RoBERTa':
    PLM_model = 'roberta-base'
    mask_token = '<mask>'
    cls_token = '<s>'
    sep_token = '</s>'
    tokenizer = RobertaTokenizer.from_pretrained(PLM_model)
    model = RobertaModel.from_pretrained(PLM_model, output_hidden_states=True)
